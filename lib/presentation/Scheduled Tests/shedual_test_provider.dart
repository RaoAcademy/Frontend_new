...
class ScheduleTestProvider with ChangeNotifier {
  ...
  getSchedualPracticeTest() async {
    final res = await http.post(
        Uri.parse(LoopsUrls.baseUrl + LoopsUrls.fschedualPractice),
        body: {'userId': '673'});
  ...
  getPausedFinishPracticeTest() async {
    final res = await http.post(
        Uri.parse(LoopsUrls.baseUrl + LoopsUrls.fPausedFinishPractice),
        body: {'userId': '673'});
    if (res.statusCode == 200) {
  ...
  getSchedualPracticeTest1() async {
    final res = await http
        .get(Uri.parse(LoopsUrls.baseUrl + LoopsUrls.fschedualPractice));
    if (res.statusCode == 200) {
      _schedualPractice = testModelFromJson(res.body);
    }
    notifyListeners();
